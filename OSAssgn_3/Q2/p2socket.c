...
int main() {
    ...
    while(1) {
        ...
        for (int i = 0; i < 10; i++) {
            char arr1[6][2] = {{arr[5*i][0], arr[5*i][1]}, {arr[5*i+1][0], arr[5*i+1][1]}, {arr[5*i+2][0], arr[5*i+2][1]}, {arr[5*i+3][0], arr[5*i+3][1]}, {arr[5*i+4][0], arr[5*i+4][1]}, {'\0', '\0'}};
            recv(connfd, &msg, sizeof(msg), 0);
            for(int j = 0; j < 5; j++){
                printf("%c%c ", arr1[j][0], arr1[j][1]);
            }
            printf("\n");
            int x = msg.arr2[4];
